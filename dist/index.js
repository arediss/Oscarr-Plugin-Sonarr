...
var SonarrPluginApi = class {
  ...
  async getEpisodes(seriesId, seasonNumber) {
    ...
    if (seasonNumber !== void 0) params.seasonNumber = seasonNumber;
  ...
  async monitorEpisodes(episodeIds, monitored) {
  ...
  async setSeasonMonitored(seriesId, seasonNumber, monitored) {
    ...
    const seasons = current.seasons.map(
      (s) => s.seasonNumber === seasonNumber ? { ...s, monitored } : s
    );
  ...
  // ── Quality ──────────────────────────────────────────────────────
  async getCutoffUnmet(page = 1, pageSize = 20) {
  ...
  // ── Releases ─────────────────────────────────────────────────────
  async getReleasesForEpisode(episodeId) {
    const { data } = await this.api.get("/release", { params: { episodeId } });
    return data;
  }
  async getReleasesForSeason(seriesId, seasonNumber) {
  ...
  async getBlocklist(page = 1, pageSize = 20) {
  ...
  async getBlocklistForSeries(seriesId) {
    const { data } = await this.api.get("/blocklist", {
  ...
  async getHistory(page = 1, pageSize = 20, eventType) {
    const params = {
      page,
      pageSize,
      sortKey: "date",
  ...
  async getQueueForSeries(seriesId) {
    const { data } = await this.api.get("/queue", {
      params: {
        page: 1,
        pageSize: 200,
...
var CACHE_TTL = 6e4;
async function getCachedSeries(api) {
  const now = Date.now();
...
function seasonsRoutes(app) {
  app.get("/series/:id/seasons", async (request) => {
    ...
    return {
      seriesId: series.id,
      seasons: (series.seasons || []).slice().sort((a, b) => a.seasonNumber - b.seasonNumber).map((s) => ({
  ...
  app.get("/series/:id/episodes", async (request) => {
    const api = request.sonarrApi;
    const { id } = request.params;
    const { seasonNumber } = request.query;
    const episodes = await api.getEpisodes(
      parseInt(id),
      seasonNumber !== void 0 ? parseInt(seasonNumber) : void 0
    );
    return { episodes };
  });
  ...
  app.put("/episodes/monitor", async (request) => {
    const api = request.sonarrApi;
    const { episodeIds, monitored } = request.body;
...
function releasesRoutes(app) {
  app.get("/releases", async (request) => {
    const api = request.sonarrApi;
    const { episodeId, seriesId, seasonNumber } = request.query;
    if (episodeId) return api.getReleasesForEpisode(parseInt(episodeId));
    if (seriesId && seasonNumber !== void 0) {
  ...
  app.post("/releases/grab", async (request) => {
    const api = request.sonarrApi;
    const { guid, indexerId } = request.body;
...
function analyticsRoutes(app) {
  app.get("/analytics", async (request) => {
    ...
    const seriesTypeCounts = { standard: 0, anime: 0, daily: 0 };
    for (const s of series) {
    ...
    const topBySize = series.map((s) => ({
      id: s.id,
      title: s.title,
      year: s.year,
      size: s.statistics?.sizeOnDisk ?? 0,
      episodeFileCount: s.statistics?.episodeFileCount ?? 0
    })).filter((s) => s.size > 0).sort((a, b) => b.size - a.size).slice(0, 10);
    const profileMap = {};
    ...
    return {
      ...
      seriesTypeCounts: Object.entries(seriesTypeCounts).map(([name, count]) => ({ name, count })).filter((x) => x.count > 0),
      ...
      rootFolders: Object.entries(rootFolders).map(([path, data]) => ({ path, ...data })).sort((a, b) => b.size - a.size),
      topBySize,
      profileMap
    };
  });
  ...
  app.get("/analytics/history", async (request) => {
    const api = request.sonarrApi;
    const { page = "1", pageSize = "50", eventType } = request.query;
...
function downloadsRoutes(app) {
  ...
  app.delete("/queue/:id", async (request) => {
    const api = request.sonarrApi;
    const { id } = request.params;
    const { removeFromClient, blocklist } = request.query;
...
var manifest = JSON.parse(readFileSync(join(__dirname, "..", "manifest.json"), "utf-8"));
var sonarrApi = null;
  ...
  return {
    ...
    async registerRoutes(app) {
      app.addHook("preHandler", async (request, reply) => {
        try {
          request.sonarrApi = await getApi(ctx);
        } catch {
